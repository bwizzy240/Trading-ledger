...
export default function TradingLedger() {
  ...
  const stats = useMemo(() => {
    if (trades.length === 0) {
      return { count: 0, winRate: 0, avgR: 0, totalR: 0, netPL: 0, profitFactor: null, curve: [] };
    }
  ...
  return (
    <div style={{ background: COLORS.bg, color: COLORS.ink, fontFamily: SERIF, minHeight: "100%", padding: "0", borderRadius: 8, overflow: "hidden" }}>
      ...
      <div style={{ padding: "24px 28px 32px 28px" }}>
        ...
        {tab === "journal" && <JournalTab trades={trades} addTrade={addTrade} deleteTrade={deleteTrade} />}
...
function JournalTab({ trades, addTrade, deleteTrade }) {
  ...
  function submit() {
    ...
    if ([entry, stop, exit].some((n) => isNaN(n)) || !contracts || contracts < 1) {
      setError("Fill in entry, stop, exit, and contracts as numbers before adding the trade.");
      return;
    }
    ...
    const rMultiple = riskPerContract === 0 ? 0 : rewardPerContract / riskPerContract;
    const ticks = rewardPerContract / preset.tick;
    const dollarPL = Math.round(ticks * preset.tickValue * contracts * 100) / 100;
    ...
    <div>
      <div style={{ background: COLORS.panel, border: `1px solid ${COLORS.rule}`, borderRadius: 4, padding: 20, marginBottom: 24 }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14 }}>
          ...
          <div>
            <label className="tl-label">Exit price</label>
            <input className="tl-input" type="number" step="any" value={form.exit} onChange={(e) => set("exit", e.target.value)} placeholder="e.g. 5930.50" />
          </div>
          <div>
      ...
      {trades.length === 0 ? (
        ...
        <table className="tl-table">
          <thead>
            <tr>
              <th>Date</th><th>Instrument</th><th>Dir</th><th>Entry</th><th>Stop</th><th>Exit</th><th>Contracts</th><th>Setup</th><th>R</th><th>P/L</th><th></th>
            </tr>
          </thead>
          <tbody>
            {trades.map((t) => (
              <tr key={t.id}>
                ...
                <td style={{ color: t.rMultiple >= 0 ? COLORS.profit : COLORS.loss }}>{fmtR(t.rMultiple)}</td>
                <td style={{ color: t.dollarPL >= 0 ? COLORS.profit : COLORS.loss }}>{fmtMoney(t.dollarPL)}</td>
                <td>
